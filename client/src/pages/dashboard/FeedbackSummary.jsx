...
const types = ["Chatbot", "Virtual Tour", "Mobile App"];

const FeedbackSummary = () => {
  const [filterState, setFilterState] = useState("All");
  const [typeState, setTypeState] = useState("Chatbot");
        ...
        <div className="mx-11 mb-2 flex items-center justify-between">
          {types.map((type) => {
            return (
              <div
                key={type}
                className={`flex cursor-pointer items-center gap-2 rounded-full px-[1.2rem] py-[0.5rem] text-[0.9rem] ${type === typeState ? (isDarkMode ? "border-2 border-base-450" : "border-2 border-base-300") : "border-2 border-transparent transition-[background] duration-150 hover:bg-secondary-200/30"} ${isDarkMode ? "text-dark-text-base-300-75" : ""} `}
