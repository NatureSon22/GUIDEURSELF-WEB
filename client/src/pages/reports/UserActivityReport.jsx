...
const UserActivityReport = () => {
  ...
  const { data: allRoles } = useQuery({
    // i want to filter the allRoles
    queryKey: ["allRoles"],
    queryFn: getAllRoleTypes,
  });

  const roles = useMemo(() => {
    return allRoles.filter((role) => role.label !== "Student");
  }, [allRoles]);
  ...
  const handleGeneratePDF = (preview = false) => {
    ...
    const addHeader = () => {
      ...
      doc.setTextColor(60);
      doc.text(
        `Generated By: ${data.firstname} ${data.lastname}`,
        0 + imgWidth + 0,
        50,
      );
      ...
      doc.setTextColor(60);
      doc.text(
        `Date Range: ${fromDate || today} - ${toDate || today} `,
        0 + imgWidth + 0,
        55,
      );
