...
const PoliciesFieldMobile = ({ isLoading }) => {
  const modules = {
    toolbar: [
      [{ header: [1, 2, 3, false] }],
  ...
  useEffect(() => {
    if (policyData) {
      setPolicy(policyData.privacy_policies_mobile);
    }
  }, [policyData]);
  ...
  const handleClickUpdate = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/general/update/675cdd2056f690410f1473b7`, {
        ...
        credentials: "include",
        body: JSON.stringify({ privacy_policies_mobile: policies }),
      });
  ...
  const handleCancel = () => {
    setEdit(false);
    setPolicy(policyData?.privacy_policies_mobile || ""); // Reset to the original value
  };
          ...
          {edit ? (
            ...
            <div className="p-2 h-full flex flex-col">
              ...
              <div className="ql-editor list-disc list-outside p-4">
                <p
                  dangerouslySetInnerHTML={{ __html: policyData.privacy_policies_mobile }}
...
PoliciesFieldMobile.propTypes = {
  isLoading: PropTypes.bool,
};

export default PoliciesFieldMobile;