...
const TermsConditionsMobile = ({ isLoading }) => {
  const modules = {
    toolbar: [
      [{ header: [1, 2, 3, false] }],
  ...
  useEffect(() => {
    if (termsData) {
      setConditions(termsData.terms_conditions_mobile);
    }
  }, [termsData]);
  ...
  const handleClickUpdate = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/general/update/675cdd2056f690410f1473b7`, {
        ...
        credentials: "include",
        body: JSON.stringify({ terms_conditions_mobile: conditions }),
      });
  ...
  const handleCancel = () => {
    setEdit(false);
    setConditions(termsData?.terms_conditions_mobile || ""); // Reset to the original value
  };
          ...
          {edit ? (
            ...
            <div className="p-2 h-full flex flex-col">
              ...
              <div className="ql-editor list-disc list-outside p-4">
                <p
                  dangerouslySetInnerHTML={{ __html: termsData.terms_conditions_mobile }}
...
TermsConditionsMobile.propTypes = {
