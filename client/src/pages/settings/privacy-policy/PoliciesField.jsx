import { useState } from "react";
import Layout from "@/components/Layout";
import PropTypes from "prop-types";
import { Button } from "@/components/ui/button";

const PoliciesField = ({ isLoading, systempolicies, onPoliciesUpdate }) => {
  const [edit, setEdit] = useState(false);
  const [policies, setPolicies] = useState(systempolicies); // Store the updated mission value

  const handleInputChange = (e) => {
    setPolicies(e.target.value); // Update the local state as the user types
  };

  const handleUpdate = async () => {
    if (policies !== systempolicies) {
      try {
        const response = await fetch("http://localhost:3000/api/general/675cdd2056f690410f1473b7", {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            privacy_policies: policies,
          }),
          credentials: "include",
        });

        if (response.ok) {
          onPoliciesUpdate(policies);
          setEdit(false); // Exit the edit mode
        } else {
          alert("Failed to update mission");
        }
      } catch (error) {
        console.error("Error updating mission:", error);
        alert("Error updating mission");
      }
    } else {
      alert("No changes to update");
    }
  };

  return (
    <Layout
      title={"Privacy Policy"}
      subtitle={"Adjust privacy settings to ensure compliance."}
      toggleEditMode={setEdit}
    >
      {isLoading ? (
        <div>Loading...</div> // You can replace this with your actual loading skeleton component
      ) : (
        <div className="space-y-4">
          {edit ? (
            <div className="w-[100%] h-[1050px] flex flex-col">
              <textarea
                value={policies} // Bind the input value to the state
                onChange={handleInputChange} // Update the state on input change
                className="p-2 h-full rounded-md border"
                placeholder="Enter the university mission"
                rows={4}
              />
            </div>
          ) : (
            <div className="w-[100%] h-[1050px] flex flex-col">
              <textarea
              value={systempolicies}
              className="p-2 h-full outline-none"
              />
            </div>
          )}

          {edit && (
            <Button onClick={handleUpdate} className="mt-4">
              Update
            </Button>
          )}
        </div>
      )}
    </Layout>
  );
};


PoliciesField.propTypes = {
  isLoading: PropTypes.bool,
  systempolicies: PropTypes.string,
  onPoliciesUpdate: PropTypes.func.isRequired,
};

export default PoliciesField;
