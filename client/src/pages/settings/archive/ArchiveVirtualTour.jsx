...
const ArchiveVirtualTour = ({userData}) => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/archived-items`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  const handleUnarchiveItem = async (itemId, itemName) => {
    try {
      ...
      if (!response.ok)
        throw new Error(
          result.message || "Item parent is also archived. Retrieve it first!",
        );
      ...
      toast({
        title: "Unsuccessful",
        description:
          error.message === "ITEM_PARENT_ARCHIVED"
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/archived-items/delete`,
        {
          method: "DELETE",
    ...
    <div className="flex flex-1 flex-col space-y-5">
      ...
      <div className="flex-1">
        <DataTable
          ...
          columnActions={{
            handleUnarchive: (itemId, itemName) =>
              handleUnarchiveItem(itemId, itemName),
          }}
        />
      </div>
        ...
        <DialogContainer openDialog={openModal}>
          <div className="flex flex-col items-center gap-5">
            ...
            <p className="text-center text-[0.91rem] font-semibold">
              Warning: Once deleted, these item in the virtual tour cannot be
              restored. Do you want to proceed?
            </p>
