...
// Create Campus
router.post("/campuses", upload.single("campus_cover_photo"), async (req, res) => {
    try {
      const {
        campus_name,
        campus_code,
...
router.put("/campuses/:id", upload.single("campus_cover_photo"), async (req, res) => {
  const { id } = req.params;
  const {
    campus_name,
    campus_code,
...
router.get("/campuses", async (req, res) => {
  try {
    const campuses = await Campus.find().select("campus_name latitude longitude");
    res.json(campuses);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
